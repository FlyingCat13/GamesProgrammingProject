...
bool UWinScreen::Initialize()
{
    ...
    // Huge green winning text.
    UTextBlock* WinTextBlock = WidgetTree->ConstructWidget<UTextBlock>();
    WinTextBlock->SetText(FText::FromString("You won"));
    WinTextBlock->Font.Size = 100;
    WinTextBlock->SetColorAndOpacity(FSlateColor(FLinearColor(0.f, 0.6f, 0.f)));
    // Positioning using slot.
